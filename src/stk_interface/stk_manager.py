...
class STKManager:
    ...
    def get_satellite_attitude(self, satellite_id: str, time_shift: float = 0) -> Optional[Dict]:
        """
        获取卫星姿态信息
        ...
        try:
            ...
            # 方法1：尝试获取姿态数据提供者
            try:
                dp_attitude = satellite.DataProviders.Item("Attitude")
                start_time = self.scenario.StartTime
                end_time = self.scenario.StartTime  # 只获取当前时间点的姿态

                result = dp_attitude.Exec(start_time, end_time)
                ...
                if result and result.DataSets.Count > 0:
                    ...
                    if dataset.RowCount > 0:
                        ...
                        attitude_data = {
                            ...
                            'pitch': float(pitch),
                            'roll': float(roll),
                            'data_source': 'STK_Attitude_DataProvider'
                        }

                        logger.debug(f"方法1成功获取卫星 {satellite_id} 姿态: "
            ...
            # 方法2：尝试获取四元数姿态
            try:
                dp_quaternion = satellite.DataProviders.Item("Quaternions")
                start_time = self.scenario.StartTime
                end_time = self.scenario.StartTime

                result = dp_quaternion.Exec(start_time, end_time)

                if result and result.DataSets.Count > 0:
                    dataset = result.DataSets.Item(0)
                    if dataset.RowCount > 0:
                        # 提取四元数
                        q1 = dataset.GetValue(0, 1)  # q1
                        q2 = dataset.GetValue(0, 2)  # q2
                        q3 = dataset.GetValue(0, 3)  # q3
                        q4 = dataset.GetValue(0, 4)  # q4
                        ...
                        attitude_data = {
                            ...
                            'data_source': 'STK_Quaternion_DataProvider'
                        }

                        logger.debug(f"方法2成功获取卫星 {satellite_id} 四元数姿态")
                        return attitude_data
            ...
            # 方法3：返回默认姿态（天底指向）
            logger.debug(f"使用默认姿态数据: {satellite_id}")
            return {
                'time': self.scenario.StartTime,
                'yaw': 0.0,
                'pitch': 0.0,
                'roll': 0.0,
                'data_source': 'default_nadir_pointing'
            }

        except Exception as e:
            logger.error(f"获取卫星 {satellite_id} 姿态失败: {e}")
            return None

    def get_satellite_velocity(self, satellite_id: str, time_shift: float = 0) -> Optional[Dict]:
        """
        获取卫星速度向量信息
        ...
        try:
            ...
            # 方法1：使用Cartesian Velocity数据提供者
            try:
                dp_velocity = satellite.DataProviders.Item("Cartesian Velocity")
                start_time = self.scenario.StartTime
                end_time = self.scenario.StartTime

                result = dp_velocity.Exec(start_time, end_time)

                if result and result.DataSets.Count > 0:
                    dataset = result.DataSets.Item(0)
                    if dataset.RowCount > 0:
                        vx = dataset.GetValue(0, 1)  # X方向速度
                        vy = dataset.GetValue(0, 2)  # Y方向速度
                        vz = dataset.GetValue(0, 3)  # Z方向速度
                        ...
                        velocity_data = {
                            ...
                            'vz': float(vz),
                            'speed': speed,
                            'data_source': 'STK_Velocity_DataProvider'
                        }

                        logger.debug(f"成功获取卫星 {satellite_id} 速度: "
                                   f"speed={speed:.1f} m/s")
                        return velocity_data

            except Exception as e1:
                logger.debug(f"速度获取方法1失败: {e1}")

            # 方法2：使用LLA Velocity数据提供者
            try:
                dp_lla_vel = satellite.DataProviders.Item("LLA Velocity")
                start_time = self.scenario.StartTime
                end_time = self.scenario.StartTime

                result = dp_lla_vel.Exec(start_time, end_time)

                if result and result.DataSets.Count > 0:
                    dataset = result.DataSets.Item(0)
                    if dataset.RowCount > 0:
                        lat_rate = dataset.GetValue(0, 1)  # 纬度变化率
                        lon_rate = dataset.GetValue(0, 2)  # 经度变化率
                        alt_rate = dataset.GetValue(0, 3)  # 高度变化率

                        velocity_data = {
                            'time': start_time,
                            'lat_rate': float(lat_rate),
                            'lon_rate': float(lon_rate),
                            'alt_rate': float(alt_rate),
                            'data_source': 'STK_LLA_Velocity_DataProvider'
                        }

                        logger.debug(f"成功获取卫星 {satellite_id} LLA速度")
                        return velocity_data

            except Exception as e2:
                logger.debug(f"速度获取方法2失败: {e2}")

            # 返回空速度数据
            logger.debug(f"无法获取卫星 {satellite_id} 速度数据")
            return None

        except Exception as e:
            logger.error(f"获取卫星 {satellite_id} 速度失败: {e}")
            return None

    def get_complete_satellite_data(self, satellite_id: str, time_shift: float = 0) -> Dict[str, Any]:
        """
        获取卫星的完整数据，包括位置、姿态、速度等
        ...
        """
        try:
            logger.debug(f"🛰️ 获取卫星 {satellite_id} 完整数据...")

            # 获取位置数据
            position_data = self.get_satellite_position(satellite_id, time_shift)

            # 获取姿态数据
            attitude_data = self.get_satellite_attitude(satellite_id, time_shift)

            # 获取速度数据
            velocity_data = self.get_satellite_velocity(satellite_id, time_shift)

            # 获取轨道参数
            orbital_data = self._get_satellite_orbital_elements(satellite_id)
            ...
            complete_data = {
                ...
                "data_quality": self._assess_data_quality(position_data, attitude_data, velocity_data)
            }

            logger.debug(f"✅ 卫星 {satellite_id} 完整数据获取成功")
            return complete_data

        except Exception as e:
            logger.error(f"❌ 获取卫星 {satellite_id} 完整数据失败: {e}")
            return {
                "satellite_id": satellite_id,
                "collection_time": datetime.now().isoformat(),
                "error": str(e),
    ...
    def get_constellation_complete_data(self, satellite_list: List[str] = None) -> Dict[str, Any]:
        """
        获取整个星座的完整数据

        Args:
            satellite_list: 卫星列表（可选，默认获取所有卫星）

        Returns:
            星座完整数据字典
        """
        try:
            logger.info("🌟 开始获取星座完整数据...")

            # 如果没有提供卫星列表，获取所有卫星
            if satellite_list is None:
                satellite_list = self.get_objects("Satellite")
                # 提取卫星名称
                satellite_list = [sat.split("/")[-1] for sat in satellite_list]

            constellation_data = {
                "collection_time": datetime.now().isoformat(),
                "satellite_count": len(satellite_list),
                "satellites": [],
            ...
            for satellite_id in satellite_list:
                try:
                    satellite_data = self.get_complete_satellite_data(satellite_id)
                    constellation_data["satellites"].append(satellite_data)

                    # 更新统计信息
                    if satellite_data.get("error"):
                        constellation_data["constellation_summary"]["failed_collections"] += 1
                    else:
                        constellation_data["constellation_summary"]["successful_collections"] += 1

                        # 统计数据质量
                        quality = satellite_data.get("data_quality", "low")
                        if quality in constellation_data["constellation_summary"]["data_quality_distribution"]:
                            constellation_data["constellation_summary"]["data_quality_distribution"][quality] += 1

                except Exception as e:
                    logger.warning(f"⚠️ 获取卫星 {satellite_id} 数据失败: {e}")
            ...
            # 计算成功率
            total = constellation_data["constellation_summary"]["total_satellites"]
            successful = constellation_data["constellation_summary"]["successful_collections"]
            success_rate = successful / total if total > 0 else 0

            constellation_data["constellation_summary"]["success_rate"] = success_rate

            logger.info(f"✅ 星座数据获取完成:")
            logger.info(f"   总卫星数: {total}")
            logger.info(f"   成功采集: {successful}")
            logger.info(f"   成功率: {success_rate:.2%}")

            return constellation_data

        except Exception as e:
            logger.error(f"❌ 获取星座完整数据失败: {e}")
            return {
                "collection_time": datetime.now().isoformat(),
                "error": str(e),
                "satellites": []
    ...
    def _get_satellite_orbital_elements(self, satellite_id: str) -> Optional[Dict[str, Any]]:
        """
        获取卫星轨道根数
        ...
        try:
            ...
            # 尝试获取经典轨道根数
            try:
                dp_classical = satellite.DataProviders.Item("Classical Elements")
                start_time = self.scenario.StartTime
                end_time = self.scenario.StartTime

                result = dp_classical.Exec(start_time, end_time)

                if result and result.DataSets.Count > 0:
                    dataset = result.DataSets.Item(0)
                    if dataset.RowCount > 0:
                        # 提取轨道根数
                        semi_major_axis = dataset.GetValue(0, 1)  # 半长轴
                        eccentricity = dataset.GetValue(0, 2)     # 偏心率
                        inclination = dataset.GetValue(0, 3)      # 倾角
                        raan = dataset.GetValue(0, 4)             # 升交点赤经
                        arg_perigee = dataset.GetValue(0, 5)      # 近地点幅角
                        true_anomaly = dataset.GetValue(0, 6)     # 真近点角
                        ...
                        orbital_data = {
                            ...
                            'data_source': 'STK_Classical_Elements'
                        }

                        logger.debug(f"成功获取卫星 {satellite_id} 轨道根数")
                        return orbital_data

            except Exception as e1:
                logger.debug(f"轨道根数获取失败: {e1}")

            return None

        except Exception as e:
            logger.debug(f"获取卫星 {satellite_id} 轨道根数失败: {e}")
            return None

    def _assess_data_quality(self, position_data: Optional[Dict],
                           attitude_data: Optional[Dict],
                           velocity_data: Optional[Dict]) -> str:
        """
        评估数据质量
        ...
        try:
            ...
            # 姿态数据质量评估
            if attitude_data and attitude_data.get("data_source") != "default_nadir_pointing":
                quality_score += 2
            elif attitude_data:
                quality_score += 1

            # 速度数据质量评估
            if velocity_data and not velocity_data.get("error"):
                quality_score += 2
