...
class AerospaceMetaTaskGantt:
            ...
            'visualization': {
                'gantt_chart': {
                    ...
                    'colors': {
                        ...
                        'virtual_meta_task': '#ff9800',
                        'virtual_meta_border': '#f57c00',
    ...
    def _draw_visible_meta_task_gantt(self, ax, visible_df, min_time, max_time):
        ...
        for idx, row in complete_visible_df.iterrows():
            ...
            if row['Type'] == 'visible_meta_task':
                ...
                linewidth = 2
                height = 0.7
            else:  # virtual_atomic_task
                color = self.colors['virtual_atomic_task']
                alpha = 0.6
                edgecolor = self.colors['virtual_atomic_border']
                linewidth = 1
        ...
        legend_elements = [
            ...
            Patch(facecolor=self.colors['virtual_atomic_task'], alpha=0.6,
                  edgecolor=self.colors['virtual_atomic_border'], label='虚拟原子任务'),
    ...
    def _generate_complete_visible_timeline(self, visible_df, y_positions, min_time, max_time):
        ...
        for combo_key in y_positions.keys():
            # 导弹ID格式: RollingThreat_XXX_XX_XXXX
            # 卫星ID格式: SatelliteXX
            # 组合格式: RollingThreat_XXX_XX_XXXX_SatelliteXX
            parts = combo_key.split('_')
            if len(parts) >= 5:  # RollingThreat + XXX + XX + XXXX + SatelliteXX
                missile_id = '_'.join(parts[:-1])  # 除了最后一部分都是导弹ID
                satellite_id = parts[-1]  # 最后一部分是卫星ID
            else:
                continue  # 跳过格式不正确的组合
